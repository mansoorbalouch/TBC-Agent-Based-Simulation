...
class Platform:
    ...
    def run_simulation(self, numSimulationMonths):
        ...
        for simulationMonth in range(1, numSimulationMonths + 1):
            ...
            for i, agent in enumerate(self.myAgents):
                if ((agent.dayOfBirth <= simulationMonth) and (agent.dayOfPassing >= simulationMonth) and (agent.purposeCategory != "Creator")):
                    aliveAgents_dict[agent.agentID] = agent.proActiveness
            ...
            for i, fundyAgent in enumerate(list(aliveAgents_dict.keys())):
                if self.myAgents[fundyAgent].strategyType == "fundy":
                        self.myAgents[fundyAgent].currentMonthAllTokensExpectedPrices_Fundy = [None]*len(self.myTokens)
                        # Compute the expected prices of all tokens
                        for j, token in enumerate(self.myTokens):
                            exp_price = sum(w * fp * (1 - self.myAgents[fundyAgent].intelligenceGap) for w, fp in zip(
                                ...
                                token.monthlyFairPrices_5years[:self.myAgents[fundyAgent].numTermsForeseen_Fundy]))
            ...
            for minute in range(numSimulationMinutesPerMonth):
                ...
                if (transactingAgentHoldings > 0 or transactingAgent.liquidity > 0):
                    ...
                    if transactingAgent.strategyType == "noisy":
                        ...
                        transactingAgent.currentMonthAllTokensExpectedPrices_Charty = [None]*len(self.myTokens)
                        # Compute the expected prices of all tokens
                        for i,token in enumerate(self.myTokens):
                        ...
                        # Calculate the price gaps
                        tokensPriceGaps = [exp_price - token.currentBuyPrice for exp_price, token in zip(transactingAgent.currentMonthAllTokensExpectedPrices_Charty, self.myTokens)]
            ...
            for i, agent_id in enumerate(aliveAgents_dict.keys()):
                if (self.myAgents[agent_id].dayOfPassing == simulationMonth):
                    self.myAgents[agent_id].tokenHoldings = None
...
p = Platform(numAgents, bondingCurve, param1, param2)
start = timeit.default_timer()
